...
export function ProposalPreview({ readOnly = false }: ProposalPreviewProps) {
  ...
  const setCurrentSection = useProposalStore((s) => s.setCurrentSection);
  const setNextSectionPending = useProposalStore((s) => s.setNextSectionPending);
  const updateSectionData = useProposalStore((s) => s.updateSectionData);
  ...
  const [editSectionKey, setEditSectionKey] = useState<SectionKey>('coverPage');
  const [editInitialData, setEditInitialData] = useState<Record<string, unknown>>({});

  const handleEditSection = (key: SectionKey) => {
    const latest = useProposalStore.getState().proposal;
    const data = (latest?.sections[key]?.data ?? {}) as Record<string, unknown>;
  ...
  const handleEditSubmit = (data: Record<string, unknown>) => {
    ...
    updateSectionData(editSectionKey, data);
    setEditOpen(false);
  };
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          {!readOnly && !isCurrentConfirmed && currentSection === 'coverPage' && (
            <div className="flex flex-col gap-2">
              <button
                onClick={() => handleEditSection('coverPage')}
                className="w-full py-2 rounded-xl text-sm font-medium transition-all bg-white border border-gray-200 hover:border-gray-300 text-gray-600 hover:text-gray-800 flex items-center justify-center gap-2"
              >
                ✏ Edit Manually
              </button>
              <button
          ...
          {!readOnly && !isCurrentConfirmed && currentSection !== 'coverPage' && (
            <div className="border-t border-dashed border-[#E85D2B]/50 pt-4">
              ...
              <button
                ...
                className="mt-3 w-full py-2 rounded-xl text-sm font-medium transition-all bg-white border border-gray-200 hover:border-gray-300 text-gray-600 hover:text-gray-800 flex items-center justify-center gap-2"
              >
                ✏ Edit Manually
              </button>
              <button
                ...
                disabled={isStreaming}
                className={clsx(
                  'mt-2 w-full py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2',
                  isStreaming
                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                    : 'bg-[#0B1220] hover:bg-[#1a2535] text-white shadow-sm active:scale-[0.99]'
                )}
          ...
          {/* Already-confirmed section — show Edit + Continue buttons so back-navigation doesn't dead-end */}
          {!readOnly && isCurrentConfirmed && !isLastSection && (
            <div className="border-t border-gray-100 pt-4 mt-1">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => handleEditSection(currentSection)}
                  className="px-4 py-2.5 rounded-xl text-sm font-medium transition-all bg-white border border-gray-200 hover:border-gray-300 text-gray-600 hover:text-gray-800 flex items-center gap-1.5 flex-shrink-0"
                >
                  ✏ Edit
                </button>
                <button
                  onClick={() => {
                    if (nextSectionMeta) setCurrentSection(nextSectionMeta.key as SectionKey);
                    setNextSectionPending(true);
                  }}
                  className="flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all bg-[#0B1220] hover:bg-[#1a2535] text-white shadow-sm active:scale-[0.99] flex items-center justify-center gap-2"
                >
                  <span>Continue</span>
                  {nextSectionMeta && (
      ...
      {/* Edit modal — accessible from any section */}
      {!readOnly && (
        <OfflineModal
          open={editOpen}
